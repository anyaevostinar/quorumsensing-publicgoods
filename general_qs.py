...
class Population:
  ...
  def makeOrg(self, org_type = "ancestor"):
    ...
    if org_type == "ancestor":
      newOrg = Organism(len(self.orgs), ai_prob = 0.0, coop_prob = 0.0, neigh_prop = 0.0)
    elif org_type == "wt":
      ...
      newOrg= Organism(len(self.orgs), empty = True)
    else:
...
parser.add_argument('--proportion_ancestor', help="the starting proportion of the population that should be the ancestor organism", type=float, default=0.01)
...
data_file.write("Update AI_Prob Coop_Prob Num_Neigh Avg_Repo_Age Total\n")
data_file.close()
...
for u in range(num_updates):
  ...
  if u%100 == 0:
    ...
    data_file.write('{} {} {} {} {} {}\n'.format(u, ai_prob, coop_prob, neigh_prop, age_avg, total))
    data_file.close()
    if total == 0:
      break
